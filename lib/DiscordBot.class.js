...
class DiscordBot {
   ...
   * @param talkingAboutMeResponse {String} The default response to the mention of the bot (just mentionned the name)
   */
  constructor({
  ...
  async handleMessage(message) {
    ...
    if (this.isToMe(message)) {
      ...
      if (foundCommands.length === 0) {
        ...
        return message.reply({ content: this.talkingToMeResponse });
      }
